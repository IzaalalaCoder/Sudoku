...
/**
 * La classe IsolatedGroups implémente l'interface IHeuristic.
 * Elle représente l'heuristique : Groupes isolés.
 * 
...
//     NombreOfCases =  nombre de cases de la  ligne/col/région courante qui ont au moins les mêmes candidats
//         Si NombredeCandidats = NomberOfCases 
//          Retirer les candidats de C dans les cases de cette ligne/col/région  qui n'ont pas au moins les mêmes candidats que C


public class IsolatedGroups implements IHeuristic {
	private final IGrid grille;

	public IsolatedGroups(IGrid grid) {
	...
	public Answer compute() {
		...
		for (int i = 0; i < grille.getSize().getSize(); i++) {
			for (int j = 0; j < grille.getSize().getSize(); j++) {
				...
				int[] t1 = isolatedGroupsLine(grille, i, j);
				if (t1 != null) {
					msg = " Sur la ligne " + (i + 1)
							+ ", si des cases ont des candidats en communs avec la case de coordonnées (" + (t1[0] + 1)
							+ "," + (t1[1] + 1) + ") avec des " +
							" candidats supplémentaires alors tu peux supprimer les candidats communs car les candidats de cette case sont restreints car ils forment un groupe avec des cases de la même ligne"
							+ "\n";
					return createAnswer(0, t1[0], t1[1], i, msg);
				}
				int[] t2 = isolatedGroupsCol(grille, i, j);
				if (t2 != null) {

					msg = " Sur la colonne " + (j + 1)
							+ ", si des cases ont des candidats en communs avec la case de coordonnées (" + (t2[0] + 1)
							+ "," + (t2[1] + 1) + ") avec des " +
							" candidats supplémentaires alors tu peux supprimer les candidats communs car les candidats de cette case sont restreints car ils forment un groupe avec des cases de la même colonne"
							+ "\n";
					return createAnswer(1, t2[0], t2[1], i, msg);
				}
				int[] t3 = isolatedGroupsRegion(grille, i, j);
				if (t3 != null) {
					msg = " Sur la région de le case (" + (t3[0] + 1) + "," + (t3[1] + 1)
							+ "), si des cases ont des candidats en communs avec cette case avec des " +
							" candidats supplémentaires alors tu peux supprimer les candidats communs car les candidats de cette case sont restreints car ils forment un groupe avec des cases de la même région"
							+ "\n";
					return createAnswer(2, t3[0], t3[1], 0, msg);
				}
			}
		}
		return null;
	...
	private int[] isolatedGroupsLine(IGrid grid, int x, int y) {
		int count = 0;
	...
	private int[] isolatedGroupsCol(IGrid grid, int x, int y) {
		int count = 0;
		boolean changeNeeded = false;
		ICase c = grille.getCase(x, y);
		if (c.getIsFixed()) {
	...
	private int[] isolatedGroupsRegion(IGrid grid, int x, int y) {
		int count = 0;
		boolean changeNeeded = false;
		ICase c = grille.getCase(x, y);
		if (c.getIsFixed()) {
